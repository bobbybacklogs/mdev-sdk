...
import { color, costCell, renderTable, setColorEnabled, statusBadge, visibleLength, wrapText, } from "./format.js";
...
async function main() {
    ...
    switch (command) {
        ...
        case "search": {
            ...
            if (!query)
                throw new Error("missing search query — usage: modelsdev search <query>");
        ...
        case "info": {
            ...
            if (!id)
                throw new Error("missing model id — usage: modelsdev info <provider/model>");
            await runInfo(id, options);
            return;
        }
        case "snapshot": {
            const result = await downloadSnapshot();
            process.stdout.write(`Snapshot written to ${color(result.dir, "green")} (${result.files.length} files) at ${color(result.generatedAt, "dim")}\n`);
            return;
        }
        default:
...
async function runProviders(options) {
    ...
    const rows = list.slice(0, options.limit).map((provider) => [
        { text: provider.id },
        { text: provider.name },
...
async function runSearch(query, options) {
    ...
    const rows = matches.slice(0, options.limit).map((match) => {
...
async function runInfo(id, options) {
    ...
    const lines = [];
    lines.push(`${color(model.name, "bold")}${ascii ? " - " : " — "}${color(`${providerId}/${model.id}`, "gray")}`);
    const kv = [];
    ...
    if (model.cost?.tiers)
        kv.push(["tiers", `${model.cost.tiers.length} tiers (context-sized)`]);
    ...
    if (model.tool_call)
        badges.push(color("[tool-call]", "cyan"));
    if (model.structured_output)
        badges.push(color("[structured-output]", "magenta"));
    if (model.attachment)
        badges.push(color("[vision]", "green"));
    if (model.temperature)
        badges.push(color("[temperature]", "blue"));
    if (model.open_weights)
        badges.push(color("[open-weights]", "green"));
    if (badges.length > 0) {
...
function writeFooter(total, limit, ascii) {
    if (total <= limit)
        return;
    const more = total - limit;
    const ellipsis = ascii ? "..." : "…";
    process.stdout.write(`${color(`${ellipsis} and ${more} more (use --limit ${more} to show more)`, "dim")}\n`);
}
/** Format a per-1M cost, with an ASCII dash for missing prices. */
function fmtCost(value, ascii) {
