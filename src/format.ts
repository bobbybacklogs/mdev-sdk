 ...
 */
export function truncate(s: string, max: number, ascii = false): string {
  if (max <= 0) return "";
  const suffix = ascii ? "..." : "…";
...
export function renderTable(
  ...
  const widths = columns.map((column, i) => {
    ...
    for (const row of rows) {
      const cell = row[i];
      if (cell) width = Math.max(width, visibleLength(cell.text));
    }
  ...
  const h = ascii ? "-" : "─";
  const v = ascii ? "|" : "│";
  const [tl, tm, tr] = ascii ? (["+", "+", "+"] as const) : (["┌", "┬", "┐"] as const);
  const [ml, mm, mr] = ascii ? (["+", "+", "+"] as const) : (["├", "┼", "┤"] as const);
  const [bl, bm, br] = ascii ? (["+", "+", "+"] as const) : (["└", "┴", "┘"] as const);

  const border = (left: string, mid: string, right: string): string =>
    left + widths.map((w) => h.repeat(w + 2)).join(mid) + right;

  const renderRow = (cells: Array<TableCell | undefined>): string => {
  ...
  const lines: string[] = [border(tl, tm, tr), header];
  if (rows.length > 0) {
...
export function statusBadge(status?: string): { text: string; color: Color[] | undefined } {
  switch (status) {
    case undefined:
    case "ga":
...
export function costCell(costInput: number | undefined, ascii = false): TableCell {
  if (costInput === undefined || Number.isNaN(costInput)) {
    return { text: ascii ? "-" : "—", color: ["gray"] };
  }
  if (costInput === 0) return { text: "$0.00", color: ["green"] };
